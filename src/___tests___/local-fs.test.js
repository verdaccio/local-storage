...
describe('Local FS test', () => {
  ...
  describe('removePackage() group', () => {
    beforeEach(() => {
      ...
      mkdirp(path.join(localTempStorage, '_toDeleteWithFiles'));
      fs.writeFileSync(path.join(localTempStorage, '_toDeleteWithFiles', 'test.txt'), 'FILE CONTENT');
    });
    ...
    test('removePackage() successful for not empty folder', done => {
      const localFs: ILocalPackageManager = new LocalFS(path.join(localTempStorage, '_toDeleteWithFiles'), logger);
      localFs.removePackage(error => {
        expect(error).toBeNull();
        done();
      });
    ...
    test('removePackage() successful for inexistent folder', done => {
